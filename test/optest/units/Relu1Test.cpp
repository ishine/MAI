...
class Relu1Test : public OperatorTest {
};

TEST_F(Relu1Test, Relu1Basic) {
    std::unique_ptr<NeuralNetwork> network = NetworkBuilder()
        .addOperator(OperatorBuilder()
            ...
            .build())
        .addTensor<float>("input", {1, 2, 2, 2}, {0,-0.8f,-0.1f,2,1,-1.f,-10.f,10})
        .addTensor<float>("output", {}, {})
        .addTensor<float>("check", {1, 2, 2, 2}, {0,-0.8f,-0.1f,1,1,-1.f,-1.f,1})
        .build();
    network->init();
    network->run();
