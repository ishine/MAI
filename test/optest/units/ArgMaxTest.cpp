...
class ArgMaxTest : public OperatorTest {
};

TEST_F(ArgMaxTest, ArgMaxNoKeepDim) {
    ArgMaxParam* param = new ArgMaxParam();
    param->keepDim = false;
...
TEST_F(ArgMaxTest, ArgMaxKeepDim) {
    ArgMaxParam* param = new ArgMaxParam();
    param->keepDim = true;
    std::unique_ptr<NeuralNetwork> network = NetworkBuilder()
        .addOperator(OperatorBuilder()
