...
public:
    ArgMin() : mAxis(0), mOuterSize(1), mInnerSize(1), mParam(NULL), mRunFirst(true) {}
    ~ArgMin() = default;
    ...
    void setParam(Param* param) override {
        mParam = reinterpret_cast<ArgMinParam*>(param);
    }
    ...
    static void argMin(const T1* inputData, T2* outputData,
    ...
    MAI_STATUS run() override {
        ...
        if (outputTensor->dataType() == DT_INT32) {
            argMin<T, int32>(inputTensor->data<T>(), outputTensor->mutableData<int32>(),
                    axisSize, mOuterSize, mInnerSize);
        } else if (outputTensor->dataType() == DT_INT64) {
            argMin<T, int64>(inputTensor->data<T>(), outputTensor->mutableData<int64>(),
                    axisSize, mOuterSize, mInnerSize);
        }
        return MAI_SUCCESS;
    }
